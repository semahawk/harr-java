parser grammar HarrParser;

options {
  output = AST; // Produce a tree of node
  tokenVocab = HarrLexer; // Use tokens defined in our lexer.
...
literal returns [Node node]:
  ...
  | NIL               { $node = new LiteralNode(HarrRuntime.getNil()); }
  | TRUE              { $node = new LiteralNode(HarrRuntime.getTrue()); }
  | FALSE             { $node = new LiteralNode(HarrRuntime.getFalse()); }
